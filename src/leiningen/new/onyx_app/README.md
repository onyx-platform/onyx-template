...
`({{app-name}}.jobs.meetup-submit-job/-main)` will build and submit a job to the cluster for you. What this job does is use the `:extract-meetup-info` catalog entry to walk through the value from meetup.com, and transform it to a segment of the shape
...
`ZOOKEEPER=$(echo $DOCKER_HOST|cut -d ':' -f 2|sed "s/\/\///g") lein run -m {{app-name}}.jobs.meetup-submit-job`
